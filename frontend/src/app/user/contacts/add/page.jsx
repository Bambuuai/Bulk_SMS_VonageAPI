...
const AddCONTACTs = () => {
	...
	const {
		...
		defaultValues: {
			contacts: [{ name: "", phone_number: "", groups: [""] }],
		},
		// resolver: yupResolver(schema)
	});
	const groupNames = useRef(null);

	useEffect(() => {
		axios
			.get(USER_ENDPOINTS.FETCH_CONTACT_GROUPS)
			.then(({ data }) => {
				if (Array.isArray(data)) {
					groupNames.current = data;
					setGroupOptions(
						data.map((group) => ({ value: group, label: group }))
					);
				}
			})
	...
	function uploadFile(e) {
		...
		if (groupNames.current?.includes(importGroup)) {
			setImportGroupError({ message: "This group already exists" });
			setLoadingFile(false);
			return;
		} else {
			setImportGroupError(null);
		}
		console.log(
			groupNames.current,
			importGroup,
			groupNames.current?.includes(importGroup)
		);

		axios
			.post(USER_ENDPOINTS.IMPORT_CONTACTS, formData, {
				headers: {
			...
			.then(({ data }) => {
				if (data.added > 0 || data.upserted > 0) {
					notify.success(
						`${
							data?.upserted + data?.added + data?.modified
				...
				if (data.modified > 0) {
					notify.success(
						`${data?.modified} contacts updated successfully.`
					);
					setFileImport([]);
				}
			})
	...
	function onSubmit({ contacts }) {
		...
		axios
			.post(USER_ENDPOINTS.ADD_CONTACTS, contacts)
			.then(({ data }) => {
				if (Array.isArray(data)) {
					notify.success(
						`Contact${
	...
	const groups = [{ value: "default", label: "Default" }];
	...
	return (
		<div className="space-y-10">
			...
			>
				<form onSubmit={handleSubmit(onSubmit)}>
					{fields.map((item, index) => (
						<div
							className="lg:grid-cols-3 md:grid-cols-2 grid-cols-1 grid gap-5 mb-8 last:mb-0"
							key={index}
						>
			...
			<div className="xl:col-span-2 col-span-1">
                ...
                onChange={(selectedOptions) => {
                  setImportGroup(selectedOptions || []);
                }}
            /> */}
						<div className="flex justify-end mt-4 space-x-4">
							...
							<Button
								...
								disabled={!fileImport.length && !importGroup}
							/>
						</div>
					</form>
				</Card>
