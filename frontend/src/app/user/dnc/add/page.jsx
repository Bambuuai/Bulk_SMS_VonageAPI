...
import { formatToVonage } from "@/utils";
...
const AddDNCs = () => {
	...
	function uploadFile(e) {
		...
		axios
			.post(USER_ENDPOINTS.IMPORT_DNC, formData, {
				headers: {
			...
			.then(({ data }) => {
				if (data.added > 0 || data.upserted > 0) {
					notify.success(
						`${
							data?.upserted + data?.added
						} DNCs imported successfully.`
					);
					setFileImport([]);
				}
			})
	...
	function onSubmit(data) {
		...
		axios
			.post(USER_ENDPOINTS.ADD_DNC, apiDnc)
			.then((response) => {
	...
	return (
		<div className="space-y-10">
			...
			<form className="xl:col-span-2 col-span-1" onSubmit={uploadFile}>
