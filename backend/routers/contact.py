...
async def update_dnc(updates: list[OptionalContactUpdates],
...
# @router.post("/import", response_model=ImportContactResponse)
# async def import_contacts(
#         file: UploadFile,
#         current_user: UserWithUUID = Depends(get_current_active_user)
# ) -> UpdateModelResponse:
...
#         try:
#             # Convert the row to a dictionary and validate using Pydantic
#             valid_data = BaseContact(**row.to_dict())
#         except ValidationError as e:
#             # Raise HTTPException with validation error details to the user
#             raise HTTPException(
#                 status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
...
#                           },
#                           "$set": {**valid_data.model_dump(exclude_unset=True), "updated_at": datetime.utcnow()},
#                       },
#                       True,
#                       )
#         )
