...
const EditContact = ({ setContactEdit, contactEdit, updateLocalContact, UPDATE_ENDPOINT }) => {
  ...
  const onSubmit = (update) => {
    ...
    console.log(update)
    axios.put(UPDATE_ENDPOINT, [prepContactForUpdate(update)]).then(({ data }) => {
      console.log(data)
      if (data.modified) {
  ...
  return (
    ...
    >
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 ">
        ...
        <div className="">
          <Button text="Update Contact" type="submit" className="btn btn-dark" isLoading={isLoading} />
        </div>
      </form>
    </Modal>
  );
